    ...
    public static void main(String[] args) throws IOException {
        ...
        var astConstructionVisitor = new CoolParserBaseVisitor<ASTNode>() {
            ...
            @Override public ASTNode visitProgram(CoolParser.ProgramContext ctx) {
                ...
                return new Prog(ctx, classes);
            }

            @Override public ASTNode visitClasss(CoolParser.ClasssContext ctx) {
                Type name = new Type(ctx, ctx.name);
                Type parent_name = new Type(ctx, ctx.parent_name);
                ...
                return new Class(ctx, name, parent_name, features);
            }
            ...
            public ASTNode visitMethod(CoolParser.MethodContext ctx) {
                ...
                return new Method(ctx, new ObjectId(ctx), formals, new Type(ctx, ctx.type), (Expr) visit(ctx.body));
            }

            @Override
            public ASTNode visitAttribute(CoolParser.AttributeContext ctx) {
                return new Attribute(ctx, new ObjectId(ctx), new Type(ctx, ctx.type), ctx.e == null ? null : (Expr) visit(ctx.e));
            }

            @Override public ASTNode visitFormal(CoolParser.FormalContext ctx) {
                return new Formal(ctx, new ObjectId(ctx), new Type(ctx, ctx.type));
            }

            @Override
            public ASTNode visitPlusMinus(CoolParser.PlusMinusContext ctx) {
                return new PlusMinus(ctx, (Expr) visit(ctx.left), ctx.operation, (Expr) visit(ctx.right));
            }

            @Override
            public ASTNode visitMultDiv(CoolParser.MultDivContext ctx) {
                return new MultDiv(ctx, (Expr) visit(ctx.left), ctx.operation, (Expr) visit(ctx.right));
            }

            @Override public ASTNode visitNew(CoolParser.NewContext ctx) {
                return new New(ctx, new Type(ctx, ctx.id));
            }

            @Override public ASTNode visitString(CoolParser.StringContext ctx) { return new Stringg(ctx); }

            @Override public ASTNode visitAssignment(CoolParser.AssignmentContext ctx) {
                return new Assign(ctx, new ObjectId(ctx), (Expr) visit(ctx.e));
            }

            @Override public ASTNode visitIsvoid(CoolParser.IsvoidContext ctx) {
                return new IsVoid(ctx, (Expr) visit(ctx.e));
            }

            @Override public ASTNode visitFalse(CoolParser.FalseContext ctx) { return new False(ctx); }

            @Override public ASTNode visitWhile(CoolParser.WhileContext ctx) {
                return new While(ctx, (Expr) visit(ctx.cond), (Expr) visit(ctx.body));
            }

            @Override public ASTNode visitInt(CoolParser.IntContext ctx) { return new Int(ctx); }

            @Override public ASTNode visitNegative(CoolParser.NegativeContext ctx) {
                return new Negative(ctx, (Expr) visit(ctx.expr()));
            }

            @Override public ASTNode visitNot(CoolParser.NotContext ctx) {
                return new Not(ctx, (Expr) visit(ctx.expr()));
            }

            @Override public ASTNode visitParen(CoolParser.ParenContext ctx) {
                return new Paren(ctx, (Expr) visit(ctx.e));
            }

            @Override public ASTNode visitTrue(CoolParser.TrueContext ctx) { return new True(ctx); }
            ...
            @Override public ASTNode visitBlock(CoolParser.BlockContext ctx) {
                ...
                return new Block(ctx, subexpressions);
            }
            ...
            @Override public ASTNode visitLet(CoolParser.LetContext ctx) {
                ...
                return new Let(ctx, locals, (Expr) visit(ctx.body));
            }

            @Override public ASTNode visitRelational(CoolParser.RelationalContext ctx) {
                return new Relational(ctx, (Expr) visit(ctx.left), ctx.operation, (Expr) visit(ctx.right));
            }

            @Override public ASTNode visitId(CoolParser.IdContext ctx) { return new Id(ctx); }
            ...
            @Override public ASTNode visitUnspecified_dispatch(CoolParser.Unspecified_dispatchContext ctx) {
                ...
                return new ImplicitDispatch(ctx, new ObjectId(ctx), formalParameters);
            }

            @Override public ASTNode visitIf(CoolParser.IfContext ctx) {
                return new If(ctx, (Expr) visit(ctx.cond), (Expr) visit(ctx.thenBranch), (Expr) visit(ctx.elseBranch));
            }
            ...
            @Override public ASTNode visitCase(CoolParser.CaseContext ctx) {
                ...
                return new Case(ctx, (Expr) visit(ctx.ref_expr), branches);
            }

            @Override
            public ASTNode visitLocal(CoolParser.LocalContext ctx) {
                return new Local(ctx, new ObjectId(ctx), new Type(ctx, ctx.type), ctx.expr() == null ? null : (Expr) visit(ctx.expr()));
            }

            @Override
            public ASTNode visitCase_branch(CoolParser.Case_branchContext ctx) {
                return new CaseBranch(ctx, new ObjectId(ctx), new Type(ctx, ctx.type), ctx.e == null ? null : (Expr) visit(ctx.e));
            }
        };
        ...
        var printVisitor = new ASTVisitor<Void>() {
            ...
            public Void visit(Class classs) {
                printIndent("class");
                indent++;
                classs.classType.accept(this);
                if (classs.parentClassType != null) {
                    classs.parentClassType.accept(this);
                }
            ...
            public Void visit(Type type) {
                printIndent(type.ctx.getText());
                return null;
            }

            @Override
            public Void visit(ObjectId objectId) {
                printIndent(objectId.ctx.getText());
                return null;
            }
            ...
            public Void visit(Int intt) {
                printIndent(intt.ctx.getText());
                return null;
            }

            @Override
            public Void visit(Stringg string) {
                printIndent(string.ctx.getText());
                return null;
            }

            @Override
            public Void visit(True t) {
                printIndent(t.ctx.getText());
                return null;
            }

            @Override
            public Void visit(False f) {
                printIndent(f.ctx.getText());
                return null;
            }
...
//        // Semantic analysis
        var definitionPassVisitor = new ClassDefinitionPassVisitor();
        var resolutionPassVisitor = new ClassResolutionPassVisitor();
        var classHierarchyValidationVisitor = new ClassHierarchyValidationVisitor();
        var resolutionVisitor = new ResolutionVisitor();

        ast.accept(definitionPassVisitor);
        ast.accept(resolutionPassVisitor);
        ast.accept(classHierarchyValidationVisitor);
